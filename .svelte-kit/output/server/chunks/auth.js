...
import { decodeProtectedHeader, jwtDecrypt, calculateJwkThumbprint, base64url, jwtVerify, SignJWT, EncryptJWT } from "jose";
...
import { pgTable, timestamp, boolean, text, serial, json, integer } from "drizzle-orm/pg-core";
...
async function getAdapter(options) {
  return getBaseAdapter(options, async (opts) => {
    const { createKyselyAdapter: createKyselyAdapter2 } = await import("./index.js").then((n) => n.i);
    ...
    const { kyselyAdapter } = await import("./index.js").then((n) => n.i);
...
const cardHands = pgTable("card_hands", {
  id: serial("id").primaryKey(),
...
const gameStatistics = pgTable("game_statistics", {
  id: serial("id").primaryKey(),
  userId: integer("user_id").references(() => users.id),
  sessionId: integer("session_id").references(() => gameSessions.id),
  gameMode: text("game_mode").notNull(),
  accuracy: integer("accuracy"),
  // Percentage of correct plays
  speed: integer("speed"),
...
const connectionString = process.env.DATABASE_URL || "postgresql://username:password@localhost:5432/bridge_strategies";
